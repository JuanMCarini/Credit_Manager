...
CREATE TABLE business_partners (ID INT PRIMARY KEY NOT NULL AUTO_INCREMENT, Name VARCHAR(255), CUIT VARCHAR(20) UNIQUE, Initials VARCHAR(2) UNIQUE NOT NULL,Email VARCHAR(100), Active BOOLEAN NOT NULL DEFAULT 1);
...
-- Table: Credits
CREATE TABLE credits (ID INT PRIMARY KEY NOT NULL AUTO_INCREMENT, Origin_ID VARCHAR(13), Disbursement_Date DATE, First_Due_Date DATE, Capital_Requested DECIMAL(22,6), Capital DECIMAL(22,6), Credit_Type_ID INT, TNA_C_IVA DECIMAL(22,6), Term INT, Client_ID INT, Organism_ID INT, Purchase_ID INT, Sale_ID INT, FOREIGN KEY (Credit_Type_ID) REFERENCES credit_types(ID), FOREIGN KEY (Client_ID) REFERENCES clients(ID), FOREIGN KEY (Organism_ID) REFERENCES organisms(ID), FOREIGN KEY (Purchase_ID) REFERENCES purchases(ID), FOREIGN KEY (Sale_ID) REFERENCES sales(ID));
