...
const Sidebar = () => {
  const { apiStatus, systemModules } = useAppStore();
  const { user } = useAuthStore();

  const isOnline = apiStatus.includes('Conectado');
  const isAdmin = user?.rol === 'Administrador' || user?.rol?.nombre === 'Administrador';

  const showCreditos = systemModules?.creditos !== false;
  const showCheques = systemModules?.cheques !== false;
  const showInversores = systemModules?.inversores !== false;
  const showFinanzas = systemModules?.finanzas !== false;

  const defaultHome = showCreditos 
    ? "/dashboard-cartera" 
    : showCheques 
      ? "/cheques" 
      : showInversores 
