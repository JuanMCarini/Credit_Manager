...
def country_nationality(name: str = "", nationality: str = "") -> int:
    ...
    filtro = (df_nationality['Name'] == name) | (df_nationality['Nationality'] == nationality)
    if df_nationality.loc[filtro].empty:
        raise ValueError(f'No esta el país/nacionalidad "{name}"/"{nationality}" en la tabla de países.')
    else:
