...
def _save(
    ...
    if save:
        ...
        # Filter zero installments:
        df = df.loc[(df["Total"].abs().round(6) != 0.0)]
...
def doc_type(doc: str | int, doc_type: DocTypes) -> int:
    """
    ------------------------------------------------------------
    doc_type()
    ------------------------------------------------------------
    Normalize and validate a document depending on its type.

    Supported rules (preserving your exact logic):
      • CUIL:
            → validated and normalized via validate_cuil()
      • DNI:
            → keep digits only
            → ensure it does not exceed 8 characters
            → convert to integer

    Parameters
    ----------
    doc : str | int
        Raw document input (string or integer).
    doc_type : DocTypes
        Document type to validate ("CUIL", "DNI", etc.).

    Returns
    -------
    int
        The normalized integer document.

    Raises
    ------
    ValueError
        If DNI contains no digits or exceeds valid length,
        or if an unsupported type is provided.
    ------------------------------------------------------------
    """

    # --- Normalize and validate according to the document type ---
    if doc_type == "CUIL":
        # validate_cuil() already returns a normalized integer
        return validate_cuil(doc)

    elif doc_type == "DNI":
        # Keep only digits and ensure valid DNI constraints
        dni = "".join(ch for ch in str(doc) if ch.isdigit())

        if not dni:
            raise ValueError("⚠️⚠️⚠️ No digits found in DNI. ⚠️⚠️⚠️")

        if len(dni) > 8:
            raise ValueError(f"⚠️⚠️⚠️ {dni} is not a valid DNI. ⚠️⚠️⚠️")
...
def get_client_balance_by_document(
    doc: int | str,
    ...
    ------------------------------------------------------------
    Normalize a document (DNI/CUIL/etc.), find the corresponding
    client, retrieve all their credits up to the given date, and
    return the full balance filtered only to those credits.
    ...
    ----------
    doc : int | str
        Document value (raw, may contain symbols).
    document_type : DocTypes
        Column used as index in the clients table ("DNI", "CUIL").
    date : str | datetime | Period
        The snapshot date for `balance()`.

    Returns
    -------
    DataFrame
        Balance of all installments belonging to that client’s credits.
    ------------------------------------------------------------
    """
    ...
    df_clts = read_table("clients", document_type)
    client_id = df_clts.at[doc, "ID"]

    # --- Get all credits for this client up to the given date ---
    df_crts = read_table("credits")
    credits_id = df_crts.loc[
        (df_crts["Client_ID"] == client_id) & (df_crts["Disbursement_Date"] <= date)
    ].index.values

    # --- Load full balance and keep only installments for these credits ---
    df = balance(date)
